...
if load_existing_model:
    ...
    date = '2024-04-22'  # 2020-07-05 run 0 for successful node agent
    run = '1'
    i_start = 3
    ...
    new_params = {'train_it': 100} # was 40000
    ...
    # Initialise hyperparameters for model
...
# Make set of environments: one for each batch (one from each session)

"""
Load HDF5
"""
h5_file_path = params['h5_path']
hf = h5py.File(h5_file_path, 'r')
env_data = hf['env_data']
session_id_list = list(set(env_data['session_id']))

# Make set of environments: one for each batch, randomly choosing to use shiny objects or not
environments = [world.World(params['world_type'], session_id, env_data) for session_id in np.random.choice(session_id_list, params['batch_size'])]
...
# Make a single walk for each environment/Session, where walk lengths are the number of trials per session
walks = []
for currSession in environments:
    walk_length = params['n_rollout']*np.random.randint(params['walk_it_min'], params['walk_it_max'])
    if walk_length > currSession.num_steps:            # TODO: find a way to work around getting longer walk lengths if necessary!!! this may be an issue when training.... // monitor how the length changes over time it may max out quickly!! ask maxime for more data in this case
        walk_length = currSession.num_steps

    walks.append(currSession.generate_walks(walk_length, 1)[0])
...
for i in range(i_start, params['train_it']):
    ...
    for env_i, walk in enumerate(walks):
        # TODO: Make sure this walk has enough steps in it for a whole backprop iteration
        ...
        for step in range(params['n_rollout']):
            ...
            else: # TODO: check that this will work for more than one environment!!
    ...
    for step in forward:
        ...
        for env_i, env_visited in enumerate(visited):
            if env_visited[step.g[env_i] -1]:
                curr_loss = torch.zeros(80)
                curr_loss[0] = step.L[0][env_i]
                curr_loss[1] = step.L[1][env_i]
                stacked0 = torch.stack([l[env_i] for l in step.L[2]])
                curr_loss[2:27] = stacked0
                stacked1 = torch.stack([l[env_i] for l in step.L[3]])
                curr_loss[27:52] = stacked1
                stacked2 = torch.stack([l[env_i] for l in step.L[4]])
                curr_loss[52:77] = stacked2
                curr_loss[77] = step.L[5][env_i]
                curr_loss[78] = step.L[6][env_i]
                curr_loss[79] = step.L[7][env_i]

                step_loss.append(loss_weights * curr_loss)

            else:
                env_visited[step.g[env_i] -1] = True
    ...
    # Log progress
    if i % 1 == 0: # ToDO make this back to 10 later
