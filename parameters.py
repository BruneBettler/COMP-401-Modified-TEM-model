...
Edited on Mon Feb 12 07:02 2024
@author: Brune
"""
...
def parameters():
    ...
    params['loss_weights_x'] = torch.ones(25) # was 1
    ...
    params['loss_weights'] = torch.zeros(80, dtype=torch.float)
    params['loss_weights'][0] = params['loss_weights_p']
    params['loss_weights'][1] = params['loss_weights_p']
    params['loss_weights'][2:27] = params['loss_weights_x']
    params['loss_weights'][27:52] = params['loss_weights_x']
    params['loss_weights'][52:77] = params['loss_weights_x']
    params['loss_weights'][77] = params['loss_weights_g']
    params['loss_weights'][78] = params['loss_weights_reg_g']
    params['loss_weights'][79] = params['loss_weights_reg_p']
    ...
    params['n_actions'] = 3  # turn_left, turn_right, forward
    # Bias for explorative behaviour to pick the same action again, to encourage straight walks
    params['explore_bias'] = 0  # TODO: this was originally 2, consider adding it back
    # Array of indices of the session (env) to use per world creation before training
    params['training_session_indices'] = [3] # TODO change to this when done debugging: np.arange(params['batch_size'])
    # Path to the world graph type to use
    params['world_type'] = 'envs/5x3bone.jason'
    # Path to the hf file containing all World/Session info
    params['h5_path'] = '/Users/brune/PycharmProjects/TEM_2D-Memory/activations_memory-cc-v2_rnn_3_3_MiniGrid-Associative-MemoryS7RMTM-v0.h5'

    # -- Model parameters
    # Decide whether to sample, or assume no noise and simply take mean of all distributions
    params['do_sample'] = False  # TODO: decide what I want here and what impact it has
    ...
    params['use_p_inf'] = False # TODO !!!! was TRUE
    ...
    params['n_g_subsampled'] = [18, 18, 15, 15, 15] # prev [10, 10, 8, 6, 6]
    ...
    params['n_x'] = 325 # previously 45
    # Neurons for compressed sensory experience x_c
    params['n_x_c'] = 100 # previously 10
...
def parameter_iteration(iteration, params):
    ...
    loss_weights = torch.zeros(80, dtype=torch.float)
    loss_weights[0] = L_p_g
    loss_weights[1] = L_p_x
    loss_weights[2:27] = L_x_gen
    loss_weights[27:52] = L_x_g
    loss_weights[52:77] = L_x_p
    loss_weights[77] = L_g
    loss_weights[78] = L_reg_g
    loss_weights[79] = L_reg_p
