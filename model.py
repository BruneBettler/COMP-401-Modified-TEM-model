...
modified by brune 2024 minigrid
"""
# Standard modules
import numpy as np
import torch
...
class Model(torch.nn.Module):
    ...
    def iteration(self, x, locations, a_prev, M_prev, x_prev, g_prev):
        ...
        if self.hyper['use_p_inf']:
            ...
            M.append(M[0] if self.hyper['common_memory'] else self.hebbian(M_prev[1], torch.cat(p_inf, dim=1),
    ...
    def loss(self, g_gen, p_gen, x_logits, x, g_inf, p_inf, p_inf_x, M_prev):
        ...
        x_unfolded = x.reshape(-1, 25, 4)
        labels = utils.from_bit_to_hot(x_unfolded)
        labels = torch.from_numpy(labels)

        # TODO: consider taking the mean loss over the 25 observation locations during backprop ?

        # L_x_gen: losses generated by generative model from g_prev -> g -> p -> x
        L_x_gen = utils.cross_entropy(x_logits[2].reshape(-1, 25, 13), labels)
        # L_x_g: Losses generated by generative model from g_inf -> p -> x
        L_x_g = utils.cross_entropy(x_logits[1].reshape(-1, 25, 13), labels)
        # L_x_p: Losses generated by generative model from p_inf -> x
        L_x_p = utils.cross_entropy(x_logits[0].reshape(-1, 25, 13), labels)
    ...
    def init_iteration(self, g, x, a, M):
        ...
        if M is None:
            ...
            M = [torch.zeros((self.hyper['batch_size'], sum(self.hyper['n_p']), sum(self.hyper['n_p'])),
                             dtype=torch.float)]

            '''# TODO: REQUIRE GRAD WAS ORIGINALLY TRUE
            ...
            if self.hyper['use_p_inf']:
                ...
                M.append(M[0] if self.hyper['common_memory'] else torch.zeros(
    ...
    def inf_g(self, p_x, g_gen, x, locations):
        ...
        shiny_envs = [False for location in locations]
        if any(shiny_envs):
...
class Iteration:
    ...
    def correct(self):
        ...
        observation_cond_raw = self.x.detach().numpy()
        # un-condense the observation from len 100 to len 325
        observation_cond = observation_cond_raw.reshape(-1, 25, 4)
        observation = utils.from_bit_to_hot(observation_cond)
        # observation is now batch_sizex25x13
        predictions = [(tensor.detach().numpy()).reshape(-1,25,13) for tensor in self.x_gen]
    ...
    def detach(self):
        ...
        # tensor.detach() for tensor in self.L

        for i, item in enumerate(self.L):
            if isinstance(item, torch.Tensor):
                # If the item is a tensor, detach it directly
                self.L[i] = item.detach()
            elif isinstance(item, list):
                # If the item is a list, iterate through the list and detach each tensor
                self.L[i] = [tensor.detach() for tensor in item]
